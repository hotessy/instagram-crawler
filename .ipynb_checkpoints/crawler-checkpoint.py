...
def usage():
    return """
        ...
        python crawler.py location -l venice -o ./output
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "mode", help="options: [posts, posts_full, profile, profile_script, hashtag, location]"
    )
    ...
    parser.add_argument("-l", "--location", help="instagram's location name")
    ...
    if args.mode in ["posts", "posts_full"]:
        ...
        ins_crawler = InsCrawler()
        arg_required("location")
        output(
            ins_crawler.get_location_posts_by_time(args.location,'2020/7/15', args.number or 100), args.output
        )
    else:
        usage()
